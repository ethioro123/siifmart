...
export const formatBeautifulLocation = (loc: string | undefined, theme: 'purple' | 'cyan' = 'purple') => {
    ...
    if (parts.length >= 3) {
        ...
        const dimThemeClass = theme === 'purple'
            ? 'bg-purple-50 dark:bg-purple-500/10 text-purple-600/80 dark:text-purple-300/80 border-purple-100 dark:border-purple-500/20'
            : 'bg-cyan-50 dark:bg-cyan-500/10 text-cyan-600/80 dark:text-cyan-300/80 border-cyan-100 dark:border-cyan-500/20';
...
export const PickJobModal: React.FC<PickJobModalProps> = ({
    ...
    return (
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-2 md:p-4 bg-black/40 dark:bg-black/80 backdrop-blur-sm">
            <div className="bg-white dark:bg-[#0f0f11] w-full max-w-4xl max-h-[95vh] md:max-h-[90vh] rounded-3xl border border-gray-200 dark:border-white/10 shadow-2xl flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">

                {/* Header */}
                <div className="relative p-3 md:p-6 border-b border-gray-100 dark:border-white/10 bg-gray-50/80 dark:bg-black/40 overflow-hidden shrink-0">
                    ...
                    <div className="relative flex justify-between items-start">
                        <div className="flex items-center gap-2 md:gap-4">
                            <div className="w-10 h-10 md:w-14 md:h-14 rounded-xl md:rounded-2xl bg-purple-100 dark:bg-purple-500/20 border border-purple-200 dark:border-purple-500/30 flex items-center justify-center text-purple-600 dark:text-purple-400 shadow-[0_0_20px_rgba(168,85,247,0.15)] shrink-0">
                                <Package size={20} className="md:w-7 md:h-7" />
                            </div>
                            <div className="min-w-0">
                                <div className="flex items-center gap-2 md:gap-3 mb-0.5 md:mb-1">
                                    <h2 className="text-sm md:text-2xl font-black text-gray-900 dark:text-white tracking-tight uppercase italic truncate">
                                        Pick Mission
                                    </h2>
                                    <span className="px-1.5 md:px-2 py-0.5 rounded-lg bg-gray-200 dark:bg-white/5 border border-gray-300 dark:border-white/10 text-[10px] md:text-xs font-mono text-gray-600 dark:text-gray-500 shrink-0">
                                        #{formatJobId(job)}
                                    </span>
                                </div>
                                <div className="flex flex-wrap items-center gap-1.5 md:gap-4 text-[9px] md:text-xs font-medium text-gray-500 dark:text-gray-400">
                        ...
                        <button onClick={onClose} aria-label="Close" className="p-1.5 md:p-2 hover:bg-gray-200 dark:hover:bg-white/10 rounded-xl text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors shrink-0">
                            <X size={20} className="md:w-6 md:h-6" />
                        </button>
                    </div>
                </div>

                <div className="flex-1 overflow-y-auto p-3 md:p-6 space-y-3 md:space-y-6 custom-scrollbar dark:bg-[radial-gradient(circle_at_50%_0%,rgba(168,85,247,0.03),transparent)]">
                    <div className="hidden md:grid grid-cols-1 md:grid-cols-3 gap-4 shrink-0">
                        <div className="bg-gray-50 dark:bg-white/[0.02] border border-gray-100 dark:border-white/5 p-4 rounded-2xl">
                            <span className="text-[10px] text-gray-500 dark:text-gray-500 font-black uppercase tracking-widest block mb-2">Overall Progress</span>
                            ...
                            <div className="w-full h-1.5 bg-gray-200 dark:bg-white/5 rounded-full overflow-hidden">
                                <div ref={(el) => { if (el) el.style.width = `${progressPercent}%`; }} className="h-full bg-purple-500 transition-all duration-1000 ease-out" />
                            </div>
                        </div>
                        <div className="bg-gray-50 dark:bg-white/[0.02] border border-gray-100 dark:border-white/5 p-4 rounded-2xl">
                            <span className="text-[10px] text-gray-500 dark:text-gray-500 font-black uppercase tracking-widest block mb-2">Job Status</span>
                            <div className="flex items-center gap-2 mt-1">
                                <div className={`w-2 h-2 rounded-full animate-pulse ${job.status === 'Completed' ? 'bg-green-500' : 'bg-purple-500'}`} />
                                <span className={`text-lg font-bold uppercase tracking-tight ${job.status === 'Completed' ? 'text-green-600 dark:text-green-400' : 'text-purple-600 dark:text-purple-400'}`}>
                                    {job.status}
                                </span>
                            </div>
                        </div>
                        <div className="bg-gray-50 dark:bg-white/[0.02] border border-gray-100 dark:border-white/5 p-4 rounded-2xl">
                            <span className="text-[10px] text-gray-500 dark:text-gray-500 font-black uppercase tracking-widest block mb-2">Job Number</span>
                            <div className="flex items-center gap-2 mt-1">
                                <Info size={16} className="text-pink-500 dark:text-pink-400" />
                    ...
                    <div>
                        <div className="flex items-center justify-between mb-2 md:mb-4 shrink-0">
                            <h3 className="font-bold text-gray-900 dark:text-white flex items-center gap-1.5 md:gap-2 text-[10px] md:text-sm uppercase tracking-widest">
                                <Box size={14} className="text-purple-500 dark:text-purple-400 md:w-4 md:h-4" />
                                Pick Manifest
                            </h3>
                            <span className="text-[8px] md:text-[10px] font-black text-gray-500 bg-gray-100 dark:bg-white/5 px-1.5 md:px-2 py-0.5 md:py-1 rounded border border-gray-200 dark:border-white/5 uppercase">
                        ...
                        <div className="space-y-2 md:space-y-3">
                            {job.lineItems?.map((item, idx) => {
                                ...
                                return (
                                    <div key={idx} className={`group relative bg-white dark:bg-white/[0.02] border ${isDone ? 'border-green-200 dark:border-green-500/20 bg-green-50 dark:bg-green-500/[0.01]' : 'border-gray-100 dark:border-white/5'} rounded-xl md:rounded-2xl p-2 md:p-4 flex flex-col md:flex-row items-start md:items-center justify-between hover:bg-gray-50 dark:hover:bg-white/[0.04] transition-all duration-300 gap-2 md:gap-0 shadow-sm dark:shadow-none`}>
                                        ...
                                        <div className="flex items-center gap-2 md:gap-5 w-full md:w-auto">
                                            ...
                                            <div className="min-w-0">
                                                <h4 className={`text-xs md:text-base font-bold tracking-tight mb-0.5 md:mb-1 group-hover:text-purple-600 dark:group-hover:text-purple-400 transition-colors truncate ${isDone ? 'line-through text-gray-500' : 'text-gray-900 dark:text-white'}`}>
                                                    {item.name || product?.name || 'Unknown SKU'}
                                                </h4>
                                                <div className="flex flex-col md:flex-row md:items-center gap-1 md:gap-3">
                                                    <span className="text-[8px] md:text-[10px] font-mono font-black text-gray-600 dark:text-gray-500 bg-gray-100 dark:bg-black/40 px-1.5 md:px-2 py-0.5 rounded border border-gray-200 dark:border-white/5 uppercase tracking-tighter w-fit">
                                                        {item.sku || product?.sku || 'NO SKU'}
                                                    </span>
                                                    {product?.barcode && (
                                                        <div className="flex items-center gap-1 md:gap-1.5 text-[8px] md:text-[10px] text-pink-600 dark:text-pink-400/80 font-bold uppercase tracking-widest w-fit">
                                        ...
                                        <div className="flex flex-col md:flex-row items-start md:items-center gap-2 md:gap-10 w-full md:w-auto mt-2 md:mt-0 pt-2 md:pt-0 border-t border-gray-100 dark:border-transparent md:border-t-0">
                                            <div className="text-left md:text-right flex flex-col items-start md:items-end w-full md:w-auto">
                                                <span className="text-[8px] md:text-[9px] text-gray-500 dark:text-gray-600 font-black uppercase tracking-[0.2em] block mb-0.5 md:mb-1">Source Bay</span>
                                            ...
                                            <div className="flex items-center justify-between md:justify-end gap-3 md:gap-6 w-full md:w-auto md:min-w-[180px]">
                                                <div className="text-left md:text-right">
                                                    <span className="text-[8px] md:text-[9px] text-gray-500 dark:text-gray-600 font-black uppercase tracking-[0.2em] block mb-0.5 md:mb-1">Required</span>
                                                    {(() => {
                                                        ...
                                                        if ((item as any).requestedMeasureQty && expected === item.expectedQty) {
                                                            ...
                                                            return <span className="text-sm md:text-lg font-mono font-black text-gray-900 dark:text-white">{(item as any).requestedMeasureQty} <span className="text-[8px] md:text-[10px] text-gray-500 font-bold uppercase tracking-widest">{unitDef}</span></span>;
                                                        }
                                                ...
                                                {isDone ? (
                                                    <div className="text-right border-l border-gray-200 dark:border-white/10 pl-4">
                                                        <span className="text-[8px] md:text-[9px] text-green-600 dark:text-green-600/60 font-black uppercase tracking-[0.2em] block mb-0.5 md:mb-1">Picked</span>
                                                        {(() => {
                                                            ...
                                                            if ((item as any).requestedMeasureQty && expected === item.expectedQty) {
                                                                ...
                                                                return <span className="text-sm md:text-lg font-mono font-black text-green-600 dark:text-green-400">{displayPicked} <span className="text-[8px] md:text-[10px] text-green-600/60 dark:text-green-500/60 font-bold uppercase tracking-widest">{unitDef}</span></span>;
                                                            }
                                                    ...
                                                    <button
                                                        ...
                                                        className="h-8 md:h-10 px-3 md:px-4 bg-purple-600 font-black text-white rounded-lg md:rounded-xl text-[8px] md:text-[10px] uppercase tracking-widest shadow-md hover:bg-purple-700 dark:shadow-[0_0_15px_rgba(168,85,247,0.3)] dark:hover:bg-purple-500 transition-all active:scale-95 disabled:opacity-50"
                                                    >
                                                        Pick
                                                    </button>
                                                )}
                ...
                {/* Footer */}
                <div className="p-3 md:p-6 border-t border-gray-100 dark:border-white/10 bg-gray-50/50 dark:bg-black/40 flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3 md:gap-4 shrink-0">
                    ...
                    <div className="flex flex-col-reverse md:flex-row items-stretch md:items-center gap-2 md:gap-3">
                        <button
                            onClick={onClose}
                            className="px-4 md:px-6 py-2 md:py-3 rounded-xl md:rounded-2xl bg-gray-200 hover:bg-gray-300 dark:bg-white/5 dark:hover:bg-white/10 text-gray-700 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white font-black text-[10px] md:text-xs uppercase tracking-widest transition-all w-full md:w-auto text-center"
                        >
                            Close Details
                        </button>
                            ...
                            <>
                                {progressPercent >= 100 ? (
                                    <button
                                        ...
                                        disabled={isSubmitting}
                                        className="justify-center px-4 md:px-8 py-2 md:py-3 bg-emerald-600 hover:bg-emerald-700 dark:hover:bg-emerald-500 text-white rounded-xl md:rounded-2xl font-black text-[10px] md:text-sm uppercase tracking-widest shadow-lg dark:shadow-[0_0_30px_rgba(16,185,129,0.3)] transition-all flex items-center gap-2 md:gap-3 active:scale-95 disabled:opacity-50"
                                    >
                                    ...
                                    <button
                                        ...
                                        disabled={isSubmitting}
                                        className="justify-center px-4 md:px-8 py-2 md:py-3 bg-purple-600 hover:bg-purple-700 dark:hover:bg-purple-500 text-white rounded-xl md:rounded-2xl font-black text-[10px] md:text-sm uppercase tracking-widest shadow-lg dark:shadow-[0_0_30px_rgba(168,85,247,0.3)] transition-all flex items-center gap-2 md:gap-3 active:scale-95 disabled:opacity-50 group"
                                    >
                                        {isSubmitting ? (
