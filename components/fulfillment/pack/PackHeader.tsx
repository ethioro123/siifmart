...
export const PackHeader: React.FC<PackHeaderProps> = ({
    ...
    return (
        <div className="bg-white dark:bg-black border border-gray-100 dark:border-white/10 md:rounded-3xl p-4 md:p-7 mb-2 md:mb-0 shadow-sm">
            <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-5 md:gap-6">
                ...
                <div className="flex flex-col md:flex-row gap-3 md:gap-4 w-full md:w-auto">
                    <div className="flex items-center justify-between md:justify-start gap-3 w-full md:w-auto">
                        <div className="bg-gray-100 dark:bg-black/40 backdrop-blur-md p-1.5 rounded-2xl border border-gray-200 dark:border-white/5 flex gap-1 shadow-inner">
                            <button onClick={() => setViewMode('Process')} className={`px-5 py-2.5 rounded-xl text-[10px] md:text-xs font-black uppercase tracking-widest transition-all ${viewMode === 'Process' ? 'bg-blue-600 text-white shadow-xl shadow-blue-600/20' : 'text-gray-400 dark:text-gray-500 hover:text-gray-900 dark:hover:text-white'}`}>Process</button>
                            <button onClick={() => setViewMode('History')} className={`px-5 py-2.5 rounded-xl text-[10px] md:text-xs font-black uppercase tracking-widest transition-all ${viewMode === 'History' ? 'bg-indigo-600 text-white shadow-xl shadow-indigo-600/20' : 'text-gray-400 dark:text-gray-500 hover:text-gray-900 dark:hover:text-white'}`}>History</button>
                        </div>
                        ...
                        <div className="relative flex-1 md:flex-none">
                            <button onClick={() => setIsPackFilterDropdownOpen(!isPackFilterDropdownOpen)} className="flex items-center justify-between md:justify-start gap-4 w-full px-5 py-3 bg-gray-100 dark:bg-black/40 backdrop-blur-md rounded-2xl border border-gray-200 dark:border-white/10 text-[10px] font-black text-gray-500 dark:hover:text-white transition-all shadow-inner h-full uppercase tracking-widest">
                                <div className="flex items-center gap-2 truncate opacity-80">
                                    <Filter size={13} className={`shrink-0 ${packJobFilter !== 'all' ? 'text-cyber-primary' : ''}`} />
                                ...
                                <>
                                    ...
                                    <div className="absolute top-full right-0 md:left-0 mt-3 w-56 bg-white dark:bg-[#0a0a0b]/90 backdrop-blur-xl border border-gray-100 dark:border-white/10 rounded-[2rem] p-2.5 shadow-2xl z-[51] animate-in fade-in slide-in-from-top-2 duration-300">
                                        {['all', 'pending', 'in-progress', 'completed'].map(status => (
                                            <button key={status} onClick={() => { setPackJobFilter(status as any); setIsPackFilterDropdownOpen(false); }} className={`w-full text-left px-5 py-3 rounded-2xl text-[10px] font-black tracking-widest uppercase transition-all flex items-center justify-between group ${packJobFilter === status ? 'bg-cyber-primary text-black' : 'text-gray-400 hover:bg-gray-100 dark:hover:bg-white/5 hover:text-gray-900 dark:hover:text-white'}`}>
                        ...
                        <div className="flex-1 min-w-[140px]">
                            <div className="relative group h-full">
                                <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none icon-pulse">
                                    <Search size={14} className="text-gray-400 group-focus-within:text-cyber-primary transition-colors" />
                                </div>
                                <input type="text" placeholder="Search Payload..." value={packSearch} onChange={(e) => setPackSearch(e.target.value)} className="w-full h-full bg-gray-100 dark:bg-black/40 border border-gray-200 dark:border-white/10 rounded-2xl pl-11 pr-5 py-3 text-gray-900 dark:text-white text-[10px] font-black uppercase tracking-widest focus:border-cyber-primary/50 focus:bg-white dark:focus:bg-black/60 outline-none transition-all placeholder:text-gray-400 shadow-inner" />
                            </div>
                        </div>
                    </div>
                </div>
