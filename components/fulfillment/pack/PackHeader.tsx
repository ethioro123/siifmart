                ...
                <div className="flex flex-col md:flex-row gap-3 md:gap-4 w-full md:w-auto">
                    <div className="flex items-center justify-between md:justify-start gap-3 w-full md:w-auto">
                        <div className="bg-[#EAE5D9] dark:bg-[#1C2620]/60 backdrop-blur-md p-1.5 rounded-2xl border border-[#E2DCCE] dark:border-[#A9CBA2]/10 flex gap-1 shadow-inner">
                            <button onClick={() => setViewMode('Process')} className={`px-5 py-2.5 rounded-xl text-[10px] md:text-xs font-black uppercase tracking-widest transition-all ${viewMode === 'Process' ? 'bg-[#2C5E3B] text-white shadow-xl shadow-[#2C5E3B]/20' : 'text-gray-400 dark:text-[#A9CBA2]/60 hover:text-gray-900 dark:hover:text-[#EAE5D9]'}`}>{t('warehouse.process')}</button>
                            <button onClick={() => setViewMode('History')} className={`px-5 py-2.5 rounded-xl text-[10px] md:text-xs font-black uppercase tracking-widest transition-all ${viewMode === 'History' ? 'bg-[#2C5E3B] text-white shadow-xl shadow-[#2C5E3B]/20' : 'text-gray-400 dark:text-[#A9CBA2]/60 hover:text-gray-900 dark:hover:text-[#EAE5D9]'}`}>{t('warehouse.history')}</button>
                        </div>

                        <div className="relative flex-1 md:flex-none">
                            <button onClick={() => setIsPackFilterDropdownOpen(!isPackFilterDropdownOpen)} className="flex items-center justify-between md:justify-start gap-4 w-full px-5 py-3 bg-gray-100 dark:bg-black/40 backdrop-blur-md rounded-2xl border border-gray-200 dark:border-white/10 text-[10px] font-black text-gray-550 dark:hover:text-white transition-all shadow-inner h-full uppercase tracking-widest">
                                <div className="flex items-center gap-2 truncate opacity-80">
                                    ...
                                    <span className="truncate">{t('warehouse.jobId').split(' ')[0]}: {packJobFilter === 'all' ? t('warehouse.allStatus') : packJobFilter}</span>
                                </div>
