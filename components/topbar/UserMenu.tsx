...
export function UserMenu({
   ...
   return (
      <div className="relative" ref={dropdownRef}>
         ...
         {isUserMenuOpen && (
            <div
               className="absolute right-0 mt-2 w-56 bg-cyber-gray border border-white/10 rounded-xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] z-[150] animate-in fade-in slide-in-from-top-2 overflow-hidden backdrop-blur-xl"
                     ...
                     {todayJobsTotal > 0 && (
                        <div className="mt-3 bg-gradient-to-br from-blue-500/10 via-cyan-500/5 to-transparent rounded-xl p-3 border border-blue-500/15">
                           <div className="flex items-center justify-between mb-2">
                              ...
                              <span className="text-[10px] font-mono font-black text-white bg-blue-500/20 px-1.5 py-0.5 rounded border border-blue-500/20">
                                 {todayJobsTotal}
                              </span>
                           </div>
                                 ...
                                 const icon = j.type === 'PUTAWAY' ? <Archive size={10} /> :
                                    j.type === 'PICK' ? <PackageCheck size={10} /> :
                                       j.type === 'RECEIVE' ? <Truck size={10} /> :
                                          <BoxIcon size={10} />;
                                 const color = j.type === 'PUTAWAY' ? 'text-blue-300 bg-blue-500/15 border-blue-500/20' :
                                    j.type === 'PICK' ? 'text-purple-300 bg-purple-500/15 border-purple-500/20' :
                                       j.type === 'RECEIVE' ? 'text-green-300 bg-green-500/15 border-green-500/20' :
