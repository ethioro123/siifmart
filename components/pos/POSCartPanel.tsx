...
export const POSCartPanel: React.FC = () => {
    ...
    // Compute unseen price updates count
    const cutoff = Date.now() - 7 * 24 * 60 * 60 * 1000;
    const unseenPriceUpdatesCount = priceUpdatedProducts.filter((p: any) => {
                    ...
                    <div className="relative mb-3 z-20">
                        ...
                        >
                            <span className="flex items-center gap-2">
                                ...
                                <span>{t('pos.terminalActions')}</span>
                                {unseenPriceUpdatesCount > 0 && (
                                    <span className="min-w-[16px] h-4 px-1 bg-amber-500 text-white text-[9px] font-black rounded-full flex items-center justify-center shadow-sm animate-pulse">
                                        {unseenPriceUpdatesCount > 99 ? '99+' : unseenPriceUpdatesCount}
                                    </span>
                                )}
                            </span>
                                ...
                                <div className="absolute bottom-full mb-2 left-0 right-0 bg-white/95 dark:bg-[#18201B] border border-[#E2DCCE] dark:border-emerald-950/40 rounded-2xl shadow-2xl p-2 z-20 grid grid-cols-2 gap-1 animate-in slide-in-from-bottom-2 duration-200">
                                    ...
                                    >
                                        <div className="relative shrink-0 flex items-center">
                                            ...
                                            {unseenPriceUpdatesCount > 0 && (
                                                <span className="absolute -top-1 -right-1 w-2.5 h-2.5 bg-amber-500 rounded-full flex items-center justify-center text-[6px] font-bold text-white shadow-sm border border-black/50">
                                                    {unseenPriceUpdatesCount > 99 ? '99+' : unseenPriceUpdatesCount}
                                                </span>
                                            )}
                                        </div>
