/**
 * WAREHOUSE TO STORE TRANSFER FLOW TEST
 * 
 * This script tests the complete transfer workflow:
 * 1. Create a transfer request from warehouse to store
 * 2. Ship the transfer (deduct stock from warehouse)
 * 3. Receive the transfer (add stock to store)
 * 4. Verify stock levels at both locations
 */
...
// Test configuration
const TEST_CONFIG = {
    warehouseId: null, // Will be fetched
    storeId: null,     // Will be fetched
...
async function authenticate() {
    log.step('Authenticating user...');

    // Try to sign in with a test user
    const { data, error } = await supabase.auth.signInWithPassword({
        email: 'admin@siifmart.com',
        password: 'admin123'
    });

    if (error) {
        log.error(`Authentication failed: ${error.message}`);
        log.info('Attempting to continue without authentication (may fail on RLS policies)');
        return null;
    }
