...
export const productsService = {
     ...
     */
    async checkBarcodeConflicts(sku: string | undefined, barcodes: string[]) {
        if (!sku || barcodes.length === 0) return;

        // Construct OR conditions for supabase search
        // Map clean and trimmed barcodes
        ...
        // Build the OR clause: barcode.eq.VAL,barcodes.cs.{VAL}
    ...
    async create(product: Omit<Product, 'id' | 'created_at' | 'updated_at'>) {
        // Collect barcodes to check for global conflicts
        ...
        if (barcodesToCheck.length > 0 && product.sku) {
            await this.checkBarcodeConflicts(product.sku, barcodesToCheck);
        }
        ...
        const dbProduct: any = {
            ...
            stock: product.stock,
            status: this._calculateStatus(product.stock || 0, product.minStock, product.status),
        ...
        if (error) {
            ...
            if (error.message.includes('column') && error.message.includes('does not exist')) {
                ...
                const mappedProduct = this._mapProduct(retryData);
                if (mappedProduct.stock !== undefined) {
                    this.handleAutoReplenish(mappedProduct).catch(err => {
        ...
        const mappedProduct = this._mapProduct(data);
        if (mappedProduct.stock !== undefined) {
            this.handleAutoReplenish(mappedProduct).catch(err => {
                console.error('❌ Background Auto-Replenishment Failed:', err);
            });
    ...
    _calculateStatus(stock: number, minStock?: number | null, currentStatus?: string) {
    ...
    async update(id: string, updates: Partial<Product>) {
        // Check for barcode updates and validate conflicts
        ...
        if (barcodeChanged) {
            ...
            // Fetch the existing product details if sku, barcode, or barcodes are not in the updates
            ...
            if (targetSku) {
                ...
                if (barcodesToCheck.length > 0) {
                    await this.checkBarcodeConflicts(targetSku, barcodesToCheck);
                }
            }
        }
        ...
        if (stockChanged || minStockChanged || statusChanged) {
            try {
                // Fetch current stock, min_stock, status to calculate new status
                const { data: current, error: fetchError } = await supabase
                    .from('products')
                ...
                if (!fetchError && current) {
                    ...
                    dbUpdates.status = this._calculateStatus(finalStock, finalMinStock, currentStatus);
                }
            } catch (e) {
    ...
    async handleAutoReplenish(product: Product) {
